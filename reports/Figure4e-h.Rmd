...
```{r plot-gene, fig.width=4, fig.height=4, message=FALSE, warning=FALSE, eval=TRUE}
...
# Select gene data
exprl2.subset <- subset(exprl2, gene %in% m.gene & ! layer %in% c("MZ_L1", "IZ_WM"))
exprl2.subset <- droplevels(exprl2.subset)
exprl2.subset$layer_type <- ifelse(exprl2.subset$layer %in% c("VZ", "SZ"),
                                   "Progenitor", "Post-mitotic")

p2 <- ggplot(data = exprl2.subset, 
             aes(x = as.numeric(age), y = 2^expr - 1, color=layer_type)) + 
  facet_grid(gene ~ ., scales = "free_y") +
  stat_summary(fun.y = mean, geom = "line", position=pd) +
  stat_summary(fun.data = mean_sdl, geom = "pointrange", position=pd) +
  xlab("Age") + 
  ylab("Expression") +
  scale_x_continuous(breaks = 1:num.timepoints, 
                     labels = levels(exprl2$age)) +
...
f.name <- paste(m.gene, collapse = "_")
ggsave(plot=p2, file = paste0("../analysis/", f.name, "_notlog.pdf"),
       width=4, height=4)
