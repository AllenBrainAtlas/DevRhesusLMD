...
# Save correlations
# write.csv(data.frame(cor.subset, pval.subset), file="../tmp/devcor2.csv")
```

# Figure 6a (left)
```{r plot-cor-summary, fig.width=3, fig.height=5}
...
for (i in 1:3) {
  for (j in (i + 1):4) {
    ...
    print(c(colnames(cor.subset)[i], colnames(cor.subset)[j], wt1))
  }
}
...
g3 <- ggplot(dev.expr2.subset, aes(x=escore, y=exprz, shape=species,
  ...
  geom_smooth(method="loess", se=FALSE, degree=1, size=2) +
